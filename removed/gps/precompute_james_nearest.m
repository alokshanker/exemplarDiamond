...
filer = '/nfs/onega_no_backups/users/ashrivas/siggraph2011/landmark_fgs.mat';
...
landmark_name{end+1} = 'cathedral';%'sir isaac cathedral';

landmark_gps{end+1} = [59.9329722; 30.362222]; % 
landmark_name{end+1} = 'station';%'moscow railway station';


landmark_gps{end+1} = [59.9404; 30.3139]; % 
landmark_name{end+1} = 'winter_palace';%'moscow Winter Palace';


%Load gps coordinates of all 6.5 million images
load('/nfs/onega_no_backups/users/ashrivas/codes/iccv2011/exemplarsvm/gps/all_gps.mat');
gps = double(gps);
...
for i = 1:length(landmark_name)
  ...
  bg = bb(1000000:end);
  subinds = round(linspace(1,length(bg),500));
  bg = bg(subinds);
