function app_files = apply_all_exemplars(...
...
%   ('trainval' or 'test') models: Input cell array of models (try models=load_all_models)
% M: The boosting Matrix (optional)
%
...
app_files = cell(length(ordering), 1);
...
for i = 1:length(ordering)
  ...
  for j = 1:length(inds{ordering(i)})
     ...
     %NOTE for ALOK, this one converts the file to RGB array
  ...
  for j = 1:length(inds{ordering(i)})
   ...
   app_files = coarse_boxes;
    ...
    res{j}.curid = curid;
    %allfiles = coarse_boxes(:,end);
    sprintf('\n ALOKS DEBG: score for this image is : ');
    %allfiles = coarse_boxes
