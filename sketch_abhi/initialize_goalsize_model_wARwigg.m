...
for a = 1:length(cx)
  for b = 1:length(cy)
    ...
    params.lpo = 10;
    [f_real,scales] = featpyramid2(I2, init_params.sbin, params);
    
    curfeats = f_real{targetlvl}(min(uu):max(uu),min(vv):max(vv),:);
    
    fprintf(1,'hg_size = [%d %d]\n',model.hg_size(1),model.hg_size(2));
    model.ws{end+1} = curfeats - mean(curfeats(:));
    model.b = 0;
    model.xs{end+1} = curfeats(:);
    %Fire inside self-image to get detection location
    [model.bbs{end+1}, model.xs{end}] = get_target_bb(model,I);
  end
end

%Normalized-HOG initialization
model.w = reshape(model.x,size(model.w)) - mean(model.x(:));
model.x = cell2mat(model.xs);
